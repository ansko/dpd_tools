...
def mmt_sheet_double_diagonals_circular(**kwargs):
    '''
    Create / Add a sigle circular sheet of MMT having thickness of 2 atoms.
    ...
    Required parameters:
        platelet_radius - radius of the sheet in atoms
            (when it is odd, atoms with single bond appear)
    ...
    Optional parameters:
        ...
        structure - if set, mmt is added into the structure, otherwise
    ...
    if 'charge' not in kwargs.keys():
        return structure
    Q = kwargs['charge']
    q_per_atom = Q / (len(structure['atoms']) - atoms_before)
    for atom in structure['atoms'].values():
        if atom['phase'] == 'filler':
            atom['charge'] = q_per_atom
    return structure
